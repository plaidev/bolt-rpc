...
class TrackClient extends Client {
  ...
  sendOnce() {
    ...
    this._sendOnceMethods[method] = cb;
    
    args.push((err, data) => {
      if (err) return cb(err);
      if (cb !== this._sendOnceMethods[method]) return cb(new Error('CancelledBySameMethod'));
      cb(null, data);
    });
  ...
  async fetch(method, data, options={}) {
    const promisifiedSuper = promisify(this.send).bind(this);
    const val = await promisifiedSuper(method, data, options);
    return val;
  }

  async fetchOnce(method, data, options={}) {
    // https://gist.github.com/6174/6062387
    const hash = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
    this._sendOnceMethods[method] = hash;
    const val = await this.fetch(method, data, options);
    if (hash !== this._sendOnceMethods[method]) throw new Error('CancelledBySameMethod');
    return val;
  }
